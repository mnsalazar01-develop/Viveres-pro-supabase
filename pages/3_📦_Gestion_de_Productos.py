...
# 3. ORGANIZACIÓN POR PESTAÑAS (TABS)
...
with t2:
    ...
    # CAMPOS DE CATEGORÍA AL FINAL DEL FORMULARIO
    categoria_sel = c1.selectbox("Categoría Principal (Orden Numérico)", ["--- Seleccionar ---"] + lista_cat, key="n_cat")
    ...
    subcategoria_sel = c2.selectbox("Subcategoría (Reactiva)", subcat_opciones, key="n_sub")
    
    forzar_guardado = st.checkbox("⚠️ Forzar el registro (Ignorar alertas de duplicados)", key="n_forzar")
...
with t3:
    if not df_p.empty:
        ...
        sel_e = st.selectbox("Selecciona un producto específico para modificar o eliminar:", list(prod_dict.keys()), key="s_e_p")
        p_e = prod_dict[sel_e]
        ...
        if b_del.button("🗑️ Eliminar Producto Definitivamente"):
            try:
                ...
                st.warning("Producto borrado de la base de datos."); st.rerun()
            except Exception as e: st.error(f"No se pudo eliminar: {e}")
    else:
        st.info("El catálogo está vacío. No hay elementos para editar.")
