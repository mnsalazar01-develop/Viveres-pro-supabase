...
# --- CONTROL DE VERSIONES ARQUITECTURA COHESIVA ---
VERSION_MODULO = "v13.1.0 - Corrección de Importación de Servidor"
...
# Cabecera oficial minimalista para auditar la actualización visual en pantalla
...
# 2. CARGA DE BASE DE DATOS MAESTRA (BACKEND LIGERO ESTABLE)
try:
...
# Universo indexado para el motor de autocompletado pasivo
...
# --- PESTAÑA 2: NUEVO PRODUCTO (SISTEMA DE ASISTENCIA RECONSTRUIDO v13.1.0) ---
with t2:
    ...
    with f1_c1:
        ...
        nombre_final = f1_c1.text_input("Nombre del Producto*", value=val_def_nombre, key=f"w_nombre_{f_idx}", placeholder="Nombre...")
    with f1_c2:
        s_mar_lookup = f1_c2.selectbox("🔍 Buscar Marca registrada (Opcional):", ["--- Es una Marca Nueva ---"] + lista_marcas_existentes, key=f"lk_mar_{f_idx}")
        val_def_marca = "" if s_mar_lookup == "--- Es una Marca Nueva ---" else s_mar_lookup
        marca_final = f1_c2.text_input("Marca del Producto", value=val_def_marca, key=f"w_marca_{f_idx}", placeholder="Marca...")
