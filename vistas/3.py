...
VERSION_MODULO = "v3.2.3 - Campos de Escritura Fijos con TAB/ENTER"
...
# --- PESTAÑA 2: NUEVO PRODUCTO (DISEÑO BLINDADO v3.2.3) ---
with t2:
    st.subheader("Formulario de Carga")
    
    # Extraemos las listas de textos de ayuda para que el usuario sepa qué palabras ya existen
    ...
    # --- FILA 1: NOMBRE Y MARCA (CAMPOS DE ESCRITURA LIBRE TOTAL CON MEMORIA DE AYUDA) ---
    f1_c1, f1_c2 = st.columns(2)
    
    with f1_c1:
        # Cuadro de escritura libre que no borra nada al dar ENTER o TAB
    ...
    with f1_c2:
        # Cuadro de escritura libre para la marca
                ...
                if subcategoria_sel != "--- Seleccionar ---" and id_cat_val is not None:
                    try:
                        ...
                        if res_id_sub.data and len(res_id_sub.data) > 0:
                            # Acceso seguro y blindado al primer registro de la respuesta
                            primera_fila = res_id_sub.data[0]
                            id_subcat_val = primera_fila.get('id_subcat')
                ...
                if esub != "--- Seleccionar ---" and v_c is not None:
                    try:
                        ...
                        if res_id_sub_e.data and len(res_id_sub_e.data) > 0:
                            v_s = res_id_sub_e.data[0].get('id_subcat')
                    except: v_s = None
