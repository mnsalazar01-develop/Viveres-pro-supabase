...
# 3. ENRUTADOR MODULAR UNIVERSAL (Nombres planos para evitar errores de tildes o emojis)
st.sidebar.title("🛒 Control Víveres")
st.sidebar.caption("Versión 3.0 Modular Sólida")

try:
    pg = st.navigation([
        st.Page("pages/1.py", title="🔔 Alertas y Ofertas", icon="🔍"),
        st.Page("pages/2.py", title="📊 Reportes Estadísticos", icon="📈"),
        st.Page("pages/3.py", title="📦 Gestión de Productos", icon="🛠️"),
        st.Page("pages/4.py", title="🗂️ Estructura (Cat/Subcat)", icon="🗂️"),
        st.Page("pages/5.py", title="🏪 Tiendas y Sucursales", icon="🏪"),
        st.Page("pages/6.py", title="📝 Registrar Ofertas", icon="📝"),
    ])
    pg.run()
except Exception as e:
