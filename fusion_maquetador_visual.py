                ...
                for i in range(0, len(items_finales_mosaico), columnas_elegidas):
                    ...
                    for idx, prod in enumerate(bloque_items):
                        with columnas_ui[idx]:
                            ...
                            html_tarjeta = f"""
                            <div style='line-height:1.2; margin-bottom:6px; height: 42px; overflow: hidden;'>
                                <b style='font-size: 0.65rem; color:#cdd6f4;'>{nombre_lbl}</b><br>
                                <span style='font-size: 0.58rem; color: #a6adc8;'>{marca_lbl} | {formato_empaque}</span>
                            </div>
                            """
                            ...
                            with st.container(border=True):
                                ...
                                if st.button("🚀 Publicar Oferta", use_container_width=True, key=f"btn_pub_{id_p_raw}_{id_campana_activa}"):
                                    if pvp_input <= 0.0:
                                        ...
                                        st.error("Falta seleccionar Ciudad.")
