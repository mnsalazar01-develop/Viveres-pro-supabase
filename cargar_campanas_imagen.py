...
# ==============================================================================
# PROGRAMA: cargar_campanas_imagen.py | PARTE 3 DE 5
# MODULO: PROCESAMIENTO GENERAL DEL POOL DE IMÁGENES Y ESTADÍSTICAS
# ==============================================================================
...
if not df_laboratorio_activo.empty and not df_p.empty:
    ...
    # Cruce estricto para limitar el mosaico a lo inyectado en la campaña activa
    ...
    if not df_lote_express.empty:
        ...
        # REGLA COMERCIAL DE INTEGRIDAD PARA SUFIJO (L)
        ...
        # Conteo y fijación inmediata de estadísticas superiores
        ...
        # Ordenamiento estricto por pasillos comerciales (Categoría -> Subcategoría -> Nombre)
...
# CABECERA DINÁMICA DE ESTADÍSTICAS AISLADA
...
# ==============================================================================
# PROGRAMA: cargar_campanas_imagen.py | PARTE 4 DE 5
# MODULO: FUNCIÓN LINEAL DE RENDERIZADO REJILLA DE MOSAICO (HTML + IMAGE UI)
# ==============================================================================
...
# ==============================================================================
# PROGRAMA: cargar_campanas_imagen.py | PARTE 5 DE 5
# MODULO: LOGICA DE AMBIENTES (A/B), PERSISTENCIA UPSERT SQL Y AUDITORÍA
# ==============================================================================

if not tiene_registros_activos:
    # ---- AMBIENTE A: INITIAL VOLCADO TOTAL (SI LA CAMPAÑA ESTÁ TOTALMENTE VACÍA) ----
    st.write("---")
    st.info(f"🚀 El contenedor ID ({id_campana_destino}) está vacío en pre_ofertas. El sistema ejecutará el algoritmo cruzado.")
            ...
            try:
                ...
                if not df_f1_listo.empty:
                    for _, fila_m in df_f1_listo.iterrows():
                        query_f1 = """
                            INSERT INTO public.pre_ofertas (id_producto, id_campana, id_super, precio_oferta_proyectado, numero_pagina, posicion_slot, alineacion, clonado_confirmado)
                            VALUES (%s, %s, %s, %s, %s, %s, 'C', False);
                        """
                        pag = int(float(fila_m["numero_pagina"])) if pd.notna(fila_m.get("numero_pagina")) else 0
                        slot = int(float(fila_m["posicion_slot"])) if pd.notna(fila_m.get("posicion_slot")) else 0
                        cur.execute(query_f1, (int(float(fila_m["id_producto"])), id_campana_destino, int(id_super_contexto), float(fila_m["precio_oferta"]), pag, slot))
                ...
                if not df_f2_listo.empty:
                    for _, fila_m in df_f2_listo.iterrows():
                        ...
                        pag = int(float(fila_m["numero_pagina"])) if pd.notna(fila_m.get("numero_pagina")) else 0
                        slot = int(float(fila_m["posicion_slot"])) if pd.notna(fila_m.get("posicion_slot")) else 0
                        cur.execute(query_f2, (int(float(fila_m["id_producto"])), id_campana_destino, int(id_super_contexto), float(fila_m["precio_oferta"]), pag, slot))
        ...
        for index_tab, cat_info in enumerate(form_categorias):
            ...
            with pestanas_ui[index_tab]:
                ...
                sub_seleccionada = st.selectbox(
                    "Refinar surtido por subcategoría:", options=opciones_sub,
                    format_func=lambda x: x["nombre"].upper(), key=f"sub_tab_react_{id_categoria_actual}"
                )
...
# --- PANEL DE ACCIONES COMERCIALES Y RESETEO ---
st.write("---")
    ...
    if st.button("🔥 Disparar Inyección Express de Todo lo Seleccionado", use_container_width=True, type="primary"):
        ...
        for id_prod, referencias in st.session_state.get("formulario_imagenes_dict", {}).items():
            ...
            if marcado_final or (id_registro_existente is not None):
                payload_rafaga.append({"id_pre_oferta": id_registro_existente, "id_producto": int(id_prod), "precio": float(precio_final), "clonado": bool(marcado_final)})
        ...
        if payload_rafaga:
            ...
            try:
                ...
                for reg in payload_rafaga:
                    if reg["id_pre_oferta"] is not None:
                        cur.execute("UPDATE public.pre_ofertas SET precio_oferta_proyectado = %s, clonado_confirmado = %s WHERE id_pre_oferta = %s;", (reg["precio"], reg["clonado"], reg["id_pre_oferta"]))
                    else:
                        cur.execute("INSERT INTO public.pre_ofertas (id_campana, id_producto, id_super, precio_oferta_proyectado, clonado_confirmado, numero_pagina, posicion_slot, alineacion) VALUES (%s, %s, %s, %s, %s, 0, 0, 'C');", (id_campana_destino, reg["id_producto"], id_super_contexto, reg["precio"], reg["clonado"]))
                conn.commit()
                st.toast("🚀 ¡Sincronización Exitosa en Neon!", icon="✅")
                st.rerun()
    ...
    if st.button("🗑️ Vaciar y Resetear esta Campaña", use_container_width=True, type="secondary"):
        if id_campana_destino > 0:
            ...
            if ejecutar_consulta_neon(query_delete, (id_campana_destino,), fetch=False, commit=True):
                ...
                st.session_state["formulario_imagenes_dict"] = {}
                st.toast("¡Campaña limpiada con éxito!", icon="🧹")
                st.rerun()

# ==============================================================================
# PROGRAMA: cargar_campanas_imagen.py | PARTE 5 DE 5 (CONTINUACIÓN)
# MODULO: BITÁCORA DE AUDITORÍA INFERIOR (RENDERIZADO FINAL)
# ==============================================================================
st.write("---")
st.markdown(f"#### 📋 3. Bitácora de Auditoría en Vivo: Campaña ID {id_campana_destino}")

if not df_lote_express.empty and not df_pool_unicos.empty:
    ...
    for _, fila_p in df_pool_unicos.iterrows():
        ...
        if not match_campana_p2.empty:
            ...
            status_inferior = "Sí (Included por Maquetar)" if es_conf_p2 else "❌ NO (EXC_REMOVIDO)"
            orden_prioridad_ram = 1 if es_conf_p2 else 2
    ...
    if not df_grid_inferior.empty:
        ...
        st.info(f"🔍 Monitoreo Relacional: Mostrando **{len(df_grid_inferior)} artículos** totales en la bitácora.")
        st.dataframe(
            df_grid_inferior, 
        ...
        st.info("Grilla vacía. Seleccione un Pasillo o Subcategoría superior para desplegar la bitácora.")
else:
    st.info("Grilla vacía. Seleccione un Pasillo o Subcategoría superior para desplegar la bitácora.")
